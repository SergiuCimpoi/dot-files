  ...
  opts = function(_, _)
    ...
    return {
      ...
      ui_select = function(fzf_opts, items)
        -- NOTE: Neovim requires `nvim_open_win({ height = ... })` to be a positive integer. [web:164]
        -- fzf-lua winopts.height can be either fraction (0-1) or fixed line count (>= 1). [web:146]
        local max_h = math.floor(vim.o.lines * 0.8 + 0.5)
        local list_h = #items + 4 -- LazyVim commonly uses +4 here. [web:156]
        ...
        return vim.tbl_deep_extend("force", fzf_opts, {
          ...
          winopts = {
            ...
            -- IMPORTANT: clamp to avoid height<=0 in small tmux panes / transient resizes. [web:164]
            height = clamp(math.floor(math.min(vim.o.lines * 0.8 - 16, list_h) + 0.5) + 16, 16, max_h),
            width = 0.5,
          ...
          winopts = {
            ...
            height = clamp(math.floor(math.min(vim.o.lines * 0.8, list_h) + 0.5), 1, max_h),
          },
        })
      end,
